...
export class AddedMigrations1744278541558 implements MigrationInterface {
    name = 'AddedMigrations1744278541558'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`transactions\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`amount\` decimal(10,2) NOT NULL, \`type\` enum ('DEBIT', 'CREDIT') NOT NULL, \`status\` enum ('PENDING', 'APPROVED', 'FAILED', 'CANCELLED') NOT NULL DEFAULT 'PENDING', \`payment_method\` enum ('CARD', 'TRANSFER') NULL, \`currency\` enum ('NGN') NULL, \`reference\` varchar(255) NOT NULL, \`note\` varchar(255) NULL, \`user_id\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`wallets\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`name\` varchar(255) NOT NULL, \`balance\` decimal(10,2) NOT NULL DEFAULT '0.00', \`currency\` enum ('NGN') NOT NULL DEFAULT 'NGN', \`description\` text NULL, \`user_id\` varchar(36) NULL, UNIQUE INDEX \`REL_92558c08091598f7a4439586cd\` (\`user_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
